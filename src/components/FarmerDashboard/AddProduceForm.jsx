...
const AddProduceForm = () => {
  ...
  const handleFileChange = (e) => {
    ...
    if (uniqueImages.length > 10) {
      Swal.fire(
        "Too Many Images",
        `You can only upload a maximum of 10 images. You currently have ${formData.images.length} images and tried to add ${validFiles.length} more.`,
        "warning"
      );
      return;
    }
    ...
    const uniquePreviews = combinedPreviews.filter((preview, index, arr) => {
      return (
        arr.findIndex(
          (p) => p.name === preview.name && p.size === preview.size
        ) === index
      );
    });
    ...
    try {
      ...
      Swal.fire({
        ...
        text:
          ...
          "Failed to add produce. Please try again.",
        icon: "error",
        confirmButtonColor: "#16a34a",
      });
    } finally {
        ...
        <div>
          ...
          <div className="mt-1">
            ...
            <p className="mt-2 text-sm text-gray-500">
              Select multiple images to add to your produce gallery. You can
              select images multiple times to build your collection. (Max 10
              images total)
            </p>
            {formData.images.length > 0 && (
              <p className="mt-1 text-sm text-green-600">
                Currently selected: {formData.images.length} image
